...
type Server struct {
	...
	// If shutdown is true, it's expected all serving status is NOT_SERVING, and will stay in NOT_SERVING.
	shutdown bool
...
func (s *Server) Watch(in *pb.HealthCheckRequest, stream pb.Health_WatchServer) error {
	...
	for {
		...
		case servingStatus := <-update:
			...
			err := stream.Send(&pb.HealthCheckResponse{Status: servingStatus})
			if err != nil {
